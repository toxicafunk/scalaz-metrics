...
import scalaz.zio.{DefaultRuntime, ZIO, TaskR }
import Server._
...
object TestMetricsService {
  ...
  def performTests[Ctx](metrics: Metrics[HttpTask[?], Ctx]): HttpTask[String] =
    for {
      ...
      _ <- ZIO.foreach(List(h(10), h(25), h(50), h(57), h(19)))(_.void)
      m <- metrics.meter(Label(Array("test", "meter"), "_"))
      _ <- ZIO.foreach(1 to 5)(i => ZIO.succeed(m(i.toDouble)))
    } yield { s"time $l ns" }

  def service[Ctx] =
    (metrics: Metrics[HttpTask[?], Ctx]) =>
      HttpRoutes.of[HttpTask] {
        case GET -> Root =>
          val m: Json = performTests(metrics).either
            .flatMap(ei => {
              ei.fold(t => jSingleObject("error", jString(s"failure encountered $t")), s => jSingleObject("time", jString(s)))
            })
