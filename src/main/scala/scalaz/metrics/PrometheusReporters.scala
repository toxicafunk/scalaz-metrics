...
object PrometheusReporters {
  ...
  implicit val prometheusReportPrinter: ReportPrinter[Summary.Timer, PrometheusMetrics] = new ReportPrinter[Summary.Timer, PrometheusMetrics] {
  ...
  val metricFamily2Json: (PrometheusMetrics, Option[String]) => List[Json] = (metrics: PrometheusMetrics, filter: Option[String]) => {
    ...
    filteredSamples.asScala
      .map(familySamples => {
        val json = familySamples.samples.asScala.map(sample => jObjectAssocList(List(sample.name -> jNumberOrNull(sample.value)))).toList
        println(s"json: $json")
    ...
    new Reporter[Summary.Timer, PrometheusMetrics, List, Json] {
      override val extractCounters: PrometheusMetrics => Filter => List[Json] =
        (metrics: PrometheusMetrics) =>
          (filter: Filter) => metricFamily2Json(metrics, filter)
